...
export default function getPrimeNumber(bitLength: number): bigint {
    if (!bitLength) throw new Error('bitLength is required');
...
function randomBigInt(bitLength: number): bigint {
    const binaryString: string = Array(bitLength)
...
function bigIntSquareRoot(number: bigint): bigint {
    if (number < 0n) throw new Error('Negative numbers are not supported');
